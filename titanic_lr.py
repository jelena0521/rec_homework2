...
#标准化
ss=StandardScaler()
train_ss=ss.fit_transform(train_features)
test_ss=ss.transform(test_features)


lr=LogisticRegression()
lr.fit(train_ss,train_y)
pred=lr.predict(test_ss)
...
# 得到决策树准确率(基于训练集)
acc_decision_tree = round(lr.score(train_ss, train_y), 6)
